...
import { NumberingTab } from '../components/NumberingTab'
...
const IMPLEMENTED = ['company', 'numbering', 'workflow', 'notifications', 'barcode', 'print-template', 'email-templates', 'data', 'audit']
...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader icon="settings" title="Settings" subtitle="Company profile, document numbering, workflow SLAs, notifications, barcodes, templates, data export & audit trail" />
      <Tabs tabs={tabs} active={active} onChange={k => nav(`/settings/${k}`)} />
      {active === 'company' && <CompanyTab canEdit={canEdit} />}
      {active === 'numbering' && <NumberingTab canEdit={canEdit} />}
