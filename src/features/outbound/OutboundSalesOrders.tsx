...
import { loadSoInvoices, invoiceDeliveryTone, invoiceDeliveryLabel } from './soInvoices'
...
function SOForm({ record, customers, warehouses, products, users, clientId, notify, canApprove, onClose, onDone }: any) {
  ...
  const approvalCleared = !!record && !preApproval(record.status)
  ...
  const save = async () => {
    ...
    try {
      ...
      const kept = lines.filter(r => r.product_id && (r as any).id)
      const added = lines.filter(r => r.product_id && !(r as any).id)
      if (record) {
        const keepIds = kept.map(r => (r as any).id)
        ...
        for (const r of kept) {
          const { error } = await (supabase as any).from('sales_order_items').update(lineVals(r)).eq('id', (r as any).id)
          if (error) throw error
        }
      }
      if (added.length) {
        const { error } = await (supabase as any).from('sales_order_items').insert(added.map(r => ({
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <Field label="Follow-up Person">
            <Combobox items={(users ?? []).map((u: any) => ({ id: u.id, label: u.full_name || u.id }))} value={h.assigned_to ?? ''} onChange={(id: string) => set({ assigned_to: id })} placeholder="Who follows this order through?" />
...
function InvoiceModal({ order, products, notify, onClose, onDone }: any) {
