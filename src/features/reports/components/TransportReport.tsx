...
type ChallanSlice = Pick<Tables<'delivery_challans'>, 'id' | 'challan_no' | 'challan_date' | 'invoice_no' | 'customer_id' | 'warehouse_id' | 'unloading_point' | 'delivery_method' | 'vehicle_id' | 'transporter_id' | 'transport_vendor' | 'courier_name' | 'courier_tracking_no' | 'total_qty' | 'delivery_cost' | 'status' | 'posted_at'>
type ItemSlice = Pick<Tables<'delivery_challan_items'>, 'challan_id' | 'product_id' | 'qty'>
...
// Fixed columns plus one numeric column per discovered product category.
type TransportRow = {
  date: string; challan_no: string; invoice: string; party: string; from: string; to: string
  mode: string; carrier: string; total: number; cost: number; status: string
} & Record<string, string | number>

export function TransportReport() {
  const { currentClientId } = useAuth()
  const [loading, setLoading] = useState(true)
  const [challans, setChallans] = useState<ChallanSlice[]>([])
  const [itemsByCh, setItemsByCh] = useState<Record<string, ItemSlice[]>>({})
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const m = <T extends { id: string }>(rows: T[] | null, f: (r: T) => string) => Object.fromEntries((rows ?? []).map(r => [r.id, f(r)]))
      setChallans(ch.data ?? [])
      setItemsByCh(by)
      setMaps({
        cust: m(cust.data, r => r.name),
        wh: m(wh.data, r => `${r.code} — ${r.name}`),
        veh: m(veh.data, r => [r.vehicle_number, r.vehicle_type].filter(Boolean).join(' · ')),
        tv: m(tv.data, r => r.name),
        cat: m(prods.data, r => r.category || 'Other')
      })
      setLoading(false)
    })
  }, [currentClientId])
  ...
  const categories = useMemo(() => {
    ...
    Object.values(itemsByCh).flat().forEach(it => set.add(maps.cat[it.product_id ?? ''] || 'Other'))
    return [...set].sort()
  }, [itemsByCh, maps.cat])

  const rows = useMemo(() => {
    const list: TransportRow[] = challans.map(c => {
    ...
    return t ? list.filter(r => [r.challan_no, r.invoice, r.party, r.carrier, r.to].some(v => String(v).toLowerCase().includes(t))) : list
  ...
  const tableCols: Column<TransportRow>[] = [
    { key: 'date', header: 'Date', accessor: r => r.date, sortable: true },
    { key: 'challan_no', header: 'Challan', accessor: r => r.challan_no, className: 'font-medium', sortable: true },
    { key: 'invoice', header: 'SAP Invoice', accessor: r => r.invoice },
    { key: 'party', header: 'Party', accessor: r => r.party, sortable: true },
    { key: 'from', header: 'Load From', accessor: r => r.from },
    { key: 'to', header: 'Offload To', accessor: r => r.to },
    { key: 'mode', header: 'Mode', accessor: r => r.mode },
    { key: 'carrier', header: 'Vehicle / Courier', accessor: r => r.carrier },
    ...categories.map(c => ({ key: c, header: c, className: 'text-right', accessor: (r: TransportRow) => r[c] ? formatNumber(Number(r[c])) : '' })),
    { key: 'total', header: 'Total', className: 'text-right font-semibold', accessor: r => formatNumber(r.total), sortable: true },
    { key: 'cost', header: 'Cost', className: 'text-right', accessor: r => r.cost ? formatNumber(r.cost) : '', sortable: true },
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4">
      ...
      <Card className="flex min-h-0 flex-1 flex-col overflow-hidden">
        <DataTable fill columns={tableCols} rows={rows} rowKey={r => r.challan_no} emptyTitle="No despatches yet" />
      </Card>
