...
export function useInboundData() {
  ...
  useEffect(() => {
    if (!clientId) return
    supabase.from('suppliers').select('id,supplier_code,name').eq('status', 'active')
      .then(({ data }) => setSuppliers((data ?? []).map(s => ({ id: s.id, label: s.supplier_code, sub: s.name }))))
    supabase.from('customers').select('id,customer_code,name').eq('status', 'active')
      .then(({ data }) => setCustomers((data ?? []).map(c => ({ id: c.id, label: c.customer_code, sub: c.name }))))
    supabase.from('warehouses').select('id,code,name').eq('status', 'active')
      .then(({ data }) => setWarehouses((data ?? []).map(w => ({ id: w.id, label: w.code, sub: w.name }))))
    supabase.from('products').select('id,material_code,name').eq('status', 'active')
      .then(({ data }) => setProducts((data ?? []).map(p => ({ id: p.id, label: p.material_code, sub: p.name }))))
    supabase.from('locations').select('id,location_code,warehouse_id')
      .then(({ data }) => setLocations((data ?? []).map(l => ({ id: l.id, label: l.location_code, extra: l.warehouse_id }))))
    supabase.from('transport_vendors').select('id,vendor_code,name').eq('status', 'active')
    ...
    supabase.from('drivers').select('id,driver_code,name').eq('status', 'active')
