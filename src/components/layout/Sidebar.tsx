...
export function Sidebar({ mobile, onNavigate }: { mobile?: boolean; onNavigate?: () => void }) {
  ...
  return (
    <aside className={cn('relative flex h-full flex-col border-r border-surface-line bg-sidebar text-sidebar-fg transition-[width] duration-300',
      ...
      <div className="flex h-[var(--header-h)] items-center gap-3 border-b border-surface-line px-4">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-brand-500 text-sm font-extrabold text-coal-900">3i</div>
        {!collapsed && <div className="leading-tight">
          <p className="font-display text-[14px] font-bold tracking-tight">3i Logistics</p>
      ...
      <div className="border-t border-surface-line px-4 py-3">
        {!collapsed
          ? <p className="text-[10px] font-medium text-sidebar-muted">v1.0 · SAP-compatible 3PL platform</p>
