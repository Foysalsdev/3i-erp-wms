...
export function Combobox({ items, value, onChange, placeholder, disabled, allowClear, className }:
  { items: ComboItem[]; value?: string; onChange: (id: string) => void
    placeholder?: string; disabled?: boolean; allowClear?: boolean; className?: string }) {
  ...
  return <SelectCombobox options={options} value={value} onChange={onChange}
    placeholder={placeholder} disabled={disabled} allowClear={allowClear} className={className} />
}
