...
export type RelationKey = 'warehouses' | 'transport_vendors' | 'vehicles' | 'zones' | 'drivers'
...
export const MASTERS: Record<string, MasterDef> = {
  products: {
    ...
    searchFields: ['name', 'material_code', 'barcode', 'model', 'color', 'china_code'],
    fields: [
      { name: 'image_url', label: 'Product Image', type: 'image' },
      { name: 'material_code', label: 'Material Code / SAP Reference Code', type: 'text', required: true, help: 'Single primary product reference. China code is shared across models, so it is NOT the identifier.' },
      { name: 'name', label: 'Material Description / Product Name', type: 'text', required: true, span2: true },
      { name: 'brand', label: 'Brand', type: 'text' },
      { name: 'model', label: 'Model / Series', type: 'text' },
      { name: 'category', label: 'Category', type: 'select', options: ['Refrigerator', 'Washing Machine', 'Microwave Oven', 'Air-Conditioner', 'Other'] },
      { name: 'capacity_value', label: 'Capacity', type: 'number' },
      { name: 'capacity_unit', label: 'Capacity Unit', type: 'select', options: ['L', 'KG', 'Ton'] },
      { name: 'compressor_type', label: 'Compressor', type: 'select', options: ['Inverter', 'Non-Inverter'] },
      { name: 'door_type', label: 'Door Type', type: 'select', options: ['Glass Door', 'Steel Door'] },
      { name: 'load_type', label: 'Load Type', type: 'select', options: ['Front Load', 'Top Load'] },
      { name: 'color', label: 'Color / Variant', type: 'text' },
      { name: 'china_code', label: 'China Code', type: 'text', help: 'Manufacturing code (shared across models). Used for serial mapping, not as primary reference.' },
    ...
    listColumns: [
      ...
      { key: 'capacity', header: 'Capacity', render: r => r.capacity_value ? `${r.capacity_value} ${r.capacity_unit ?? ''}` : '—' },
      { key: 'color', header: 'Color', accessor: r => r.color },
