...
// Search hits come from a handful of tables via a dynamic, comma-joined
// column list (`cols`), so the row shape can't be inferred from the schema
// at each call site — every field here is a plain scalar (id/code/name/etc).
type SearchRow = Record<string, string | null | undefined>
...
const TXNS = [
  { table: 'sales_orders', icon: 'shopping_cart', cat: 'Order', idField: 'so_no',
    ...
    sub: (r: SearchRow) => r.reference_no ? `PO ${r.reference_no}` : r.sap_so_no ? `SAP SO ${r.sap_so_no}` : '', path: '/outbound/sales-order' },
  { table: 'delivery_challans', icon: 'receipt', cat: 'Delivery Challan', idField: 'challan_no',
    ...
    sub: (r: SearchRow) => r.courier_tracking_no ? `CN ${r.courier_tracking_no}` : r.invoice_no ? `Invoice ${r.invoice_no}` : r.po_no ? `PO ${r.po_no}` : '', path: '/outbound/delivery-challan' },
  { table: 'gate_passes', icon: 'door_front', cat: 'Gate Pass', idField: 'gate_pass_no',
    ...
    sub: (r: SearchRow) => r.driver_name || r.purpose || '', path: '/outbound/gate-pass' },
  { table: 'proof_of_delivery', icon: 'task_alt', cat: 'Customer POD', idField: 'pod_no',
    ...
    sub: (r: SearchRow) => r.received_by ? `Received by ${r.received_by}` : '', path: '/outbound/pod-upload' },
  { table: 'courier_shipments', icon: 'local_post_office', cat: 'Courier / CN', idField: 'shipment_no',
    ...
    sub: (r: SearchRow) => r.tracking_no ? `CN ${r.tracking_no}` : '', path: '/transport/courier' },
  { table: 'serial_numbers', icon: 'qr_code_2', cat: 'Serial', idField: 'serial_no',
    ...
    sub: (r: SearchRow) => [r.status, r.reference_no].filter(Boolean).join(' · '), path: '/inventory/serials' }
] as const
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      const vehicleHits = (async (): Promise<Hit[]> => {
        ...
        ;(gp ?? []).forEach(g => out.push({ cat: 'Vehicle', icon: 'door_front', label: g.gate_pass_no, sub: `Gate Pass · Vehicle ${vmap[g.vehicle_id ?? ''] ?? ''}`, path: `/outbound/gate-pass?q=${encodeURIComponent(g.gate_pass_no)}` }))
        return out
      })()
